...
{ 
  ...
  {  //cout<<"ENTRE--------------------"<<endl;
	...
	//cout<<"crea punto: "<<x<<" ,  "<<y<<endl;
  	if(flag==0)
  	{
  		xi=x-tamanho;//conversion(x);
  		yi=tamanho-(y-0);//conversion(y);;
  		flag=1;
  	...
  	{
  		xf=x-tamanho;//conversion(x);
  		yf=tamanho-(y-0);//conversion(y);
  		flag=0;
...
void glPaint(void) {
	...
	glOrtho(-tamanho,  tamanho,-tamanho, tamanho, -1.0f, 1.0f); 
...
int main(int argc, char** argv) {
	...
	glutInitWindowSize(tamanho*2, tamanho*2); //tamaño de la ventana

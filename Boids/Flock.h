...
void Flock::initialize(){
    ...
    vector3d t1(75,400,75);
     vector3d t2(125,300,325);
     vector3d t3(325,200,275);
     vector3d t4(225,225,225);
     Obstaculos.push_back(t1);
     Obstaculos.push_back(t2);
     Obstaculos.push_back(t3);
     Obstaculos.push_back(t4);
    //CREANDO LOS BOIDS 
...
void Flock::drawObstaculo(){
  // glColor3f(1.0f,1.0f,1.0f);
    for(int i=0;i<Obstaculos.size();++i){
            ...
            glBindTexture(GL_TEXTURE_2D, obst);
            glPushMatrix();
                glTranslatef(Obstaculos[i].x,Obstaculos[i].y,Obstaculos[i].z);
                glRotatef(90,1,0,0);
                gluCylinder(quadratic,20,20,350,20,20);
            glPopMatrix();
        /*glPushMatrix();
            glTranslatef(Obstaculos[i].x,Obstaculos[i].y,Obstaculos[i].z);
             glutSolidCube(100);
...
void Flock::asignarTexturas(){
	for (int i = 0; i < Flocks.size(); i++) 
         for(int j=0;j<Flocks[i].size();j++)
             for(int k=0;k<Flocks[i][j].size();k++){
             	     if(Flocks[i][j][k]->boids.size()!=0){
	                     for(int m=0;m<Flocks[i][j][k]->boids.size();++m){
	                          Flocks[i][j][k]->boids[m]->TexturaPez=peces;
	                       }
	                    }
	            }
...
void Flock::drawBoids(){
	...
	/*cout<<"pez "<<peces<<endl;
    cout<<"obstaculo "<<obst<<endl;
    cout<<"despues del Flocks "<<obst<<endl;*/
     drawObstaculo();
     drawPecera();	
    for (int i = 0; i < Flocks.size(); i++) 
         for(int j=0;j<Flocks[i].size();j++)
